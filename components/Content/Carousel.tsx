      ...
      <div class="carousel carousel-start gap-4 lg:gap-8 row-start-2 row-end-5 min-h-[240px] h-64 flex items-end">
        <div class="flex m-auto gap-4">
          {section.map(({ title, image, alt, href }) => (
            <div class="border border-purple-500 border-opacity-10 rounded-t-full rounded-b-2xl h-56 w-48 p-4 hover:shadow-xl hover:-translate-y-4 transition duration-300 relative">
              <a href={href} class="flex flex-col justify-between">
                <img
                  src={image}
                  alt={alt}
